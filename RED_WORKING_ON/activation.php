...
if (isset($_GET['id']) && isset($_GET['u']) && isset($_GET['e']) && isset($_GET['p'])) {
	...
	$sql = "SELECT * FROM users WHERE user_id='$id' AND username='$u' AND email='$e' AND password='$p' LIMIT 1";
	...
	$sql = "SELECT * FROM users WHERE user_id='$id' AND activated='1' LIMIT 1";
    $query = mysqli_query($db_conx, $sql);
	$numrows = mysqli_num_rows($query);
